...
class Scheduler {
  ...
  createFileSource(id, audioPath, effectChainString) {
    this.sources[id] = new SourceFile(this.audioContext, this.config, {
      file: audioPath,
    });
  ...
  createPattern(patternId, tabId, sourceId) {
    ...
    if (
      (tabClass === "RhythmicTablature" && sourceClass !== "SourceFile") ||
      (tabClass === "MelodicTablature" && sourceClass !== "SourceOsc")
    ) {
      console.error(`Incompatible tablature and source type`);
      return;
    }

    this.patterns[patternId] = new Pattern(
      this.tablatures[tabId],
  ...
  schedule() {
    ...
    for (let i = 0; i < this.tracks.length; i++) {
      ...
      if (endTime - currentTime < 0) {
        this.sync();
        console.error(
          "out-of-sync 🤔: please re-run the script or re-load page!"
        );
      }
